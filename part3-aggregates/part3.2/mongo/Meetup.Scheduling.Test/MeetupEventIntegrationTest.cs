...
namespace Meetup.Scheduling.Test
{
    ...
    public static class MeetupSchedulingTestExtensions
    {
        ...
        public static Task<HttpResponseMessage> CreateAttendantList(this HttpClient client, string eventId,
            int capacity = Capacity)
            => client.Post("attendants", new CreateAttendantList(eventId, capacity));

        public static Task<HttpResponseMessage> UpdateTitle(this HttpClient client, string eventId, string title,
            string description)
            => client.Put($"events/details", new UpdateDetails(eventId, title, description));
        
        public static Task<HttpResponseMessage> Schedule(this HttpClient client, string eventId, DateTimeOffset start, DateTimeOffset end)
            => client.Put($"events/schedule", new Schedule(eventId, start, end));
        
        public static Task<HttpResponseMessage> MakeOnline(this HttpClient client, string eventId, string url)
            => client.Put($"events/makeonline", new MakeOnline(eventId, url));

        public static Task<HttpResponseMessage> Publish(this HttpClient client, string eventId)
            => client.Put($"events/publish", new Publish(eventId));

        public static Task<HttpResponseMessage> AcceptInvitation(this HttpClient client, string eventId,
            Guid userId)
            => client.Put($"attendants/accept", new AcceptInvitation(eventId, userId));

        public static Task<HttpResponseMessage> DeclineInvitation(this HttpClient client, string eventId,
            Guid userId)
            => client.Put($"attendants/decline", new DeclineInvitation(eventId, userId));

        public static async Task<string> ThenOk(this Task<HttpResponseMessage> httpResponseMessage)
        {
        ...
        public static async Task<MeetupEvent> Get(this HttpClient client, string eventId)
        {
