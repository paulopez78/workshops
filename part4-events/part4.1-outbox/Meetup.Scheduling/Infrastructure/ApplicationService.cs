...
namespace Meetup.Scheduling.Infrastructure
{
    ...
    public abstract class ApplicationService<T> : IApplicationService where T : Aggregate
    {
        private MeetupRepository<T> Repository;

        protected ApplicationService(MeetupRepository<T> repository) => Repository = repository;

        protected async Task<CommandResult> HandleCreate(T aggregate, CommandContext context)
        {
    ...
    public class CommandContext
    {
        ...
        public static CommandContext From(IHeaderDictionary context)
            => new(Guid.Parse(context["Idempotency-Key"]));
        ...
        public async Task<CommandResult> Handle(Guid aggregateId, object cmd, CommandContext context)
        {
            ...
            if (!loadedOutbox.Any())
            {
                ...
                var outboxMessages = context.PendingChanges
                    .Select(change => OutBox.From(context.MessageId, aggregateId, change))
                    .ToList();
            ...
            Task TryDispatch(IEnumerable<OutBox> outbox)
                => Task.WhenAll(
                    outbox
                        ...
                        .Select(x => PublishEndpoint.Publish(x.Change))
                );
    ...
    public record OutBox(Guid MessageId, Guid AggregateId, string MessageType, string Payload)
    {
        ...
        public static OutBox From(Guid messageId, Guid aggregateId, object @event)
            => new(messageId, aggregateId, @event.GetType().ToString(), JsonSerializer.Serialize(@event));
    }
