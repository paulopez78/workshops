...
namespace Meetup.Scheduling.Application.AttendantList
{
    ...
    {
        readonly MongoDbRepository<AttendantListAggregate> Repository;
        readonly UtcNow                                    GetUtcNow;

        public AttendantListApplicationService(MongoDbRepository<AttendantListAggregate> repository, UtcNow getUtcNow)
        {
        ...
        async Task<CommandResult> Handle(string id, Action<AttendantListAggregate> action)
        {
